  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error: dbErr } = await supabase.from("containers").insert({
        ...
        available_from: departureDate || new Date().toISOString(),
        available_until: arrivalDate || new Date(Date.now() + 30*86400000).toISOString(),
        capacity_kg: parseFloat(maxWeightKg) || 0,
        price_usd: parseFloat(pricePerCBM) || 0,
        ...
        refrigerated, hazmat, hazmat_approved: hazmat, accepts_partial_bookings: acceptsPartial,
        min_booking_cbm: parseFloat(minBookingCBM) || 1,
        price_per_cbm: parseFloat(pricePerCBM),
        price_per_kg: parseFloat(pricePerKg) || null,
        notes: notes || null, description: notes || null, status: "active",
