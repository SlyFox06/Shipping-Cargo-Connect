...
export const ProviderAnalytics = ({ providerId, userId }: ProviderAnalyticsProps) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`provider-analytics-${providerId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bookings' }, () => fetchAnalytics())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'containers' }, () => fetchAnalytics())
      .subscribe();
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Get bookings for provider's containers
      ...
      const avgUtil = containers.length > 0
        ? containers.reduce((sum, c) => sum + (Number(c.utilization_rate) || 0), 0) / containers.length
        : 0;

      const totalRevenue = bookings.reduce((sum, b) => sum + Number(b.price_usd), 0);
      const activeContainers = containers.filter(c => c.status === 'available').length;
      ...
      // Revenue by route
      const routeRevenue = containers.reduce((acc: any, container) => {
        const route = `${container.origin} → ${container.destination}`;
        const routeBookings = bookings.filter(b => b.container_id === container.id);
        const revenue = routeBookings.reduce((sum, b) => sum + Number(b.price_usd), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <SavedChartsList userId={userId} userRole="provider" />
